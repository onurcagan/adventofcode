...
func main() {
	log.SetFlags(0)
	QuestionSolver(dataVar)
}

func QuestionSolver(str string) {
	var test = s.Split(str, "\n")
	sum := 0
	for i := 0; i < len(test); i++ {
		var numsInWord []int
		// var sum int
		for j := 0; j < len(test[i]); j++ {
			var currentChar = rune(test[i][j])

			if u.IsDigit(currentChar) {
				digitalValue := int(currentChar - '0')
				numsInWord = append(numsInWord, digitalValue)
			}
		}

		if len(numsInWord) != 0 {

			total, err := combine(numsInWord[0], numsInWord[len(numsInWord)-1])
...
func combine(a int, b int) (int, error) {
	combinedValue := fmt.Sprintf("%d%d", a, b)
	res, err := strconv.Atoi(combinedValue)

	if err != nil {
		return 0, errors.New("something broke yo!, couldn't convert str to int")
	}
